...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        for i in range(n_blocks-blo):       # add ResNet blocks
...
class D_attn(nn.Module):
    def __init__(self, weight_dim=1728):
        super().__init__()

        self.gap_fc = nn.utils.spectral_norm(nn.Linear(weight_dim, 1, bias=False))
        self.gmp_fc = nn.utils.spectral_norm(nn.Linear(weight_dim, 1, bias=False))
        self.conv1x1 = nn.Conv2d(weight_dim * 2, weight_dim, kernel_size=1, stride=1, bias=True)
        self.leaky_relu = nn.LeakyReLU(0.2, True)

        self.pad = nn.ReflectionPad2d(1)
        self.conv = nn.utils.spectral_norm(
            nn.Conv2d(weight_dim, 1, kernel_size=4, stride=1, padding=0, bias=False))
...
class NCE_attn(nn.Module):
    def __init__(self, weight_dim0=3, weight_dim1=128, weight_dim2=256):
        super().__init__()

        self.qfc_one = nn.utils.spectral_norm(nn.Linear(weight_dim0, 1, bias=False))
        self.qfc_two = nn.utils.spectral_norm(nn.Linear(weight_dim1, 1, bias=False))
        self.qfc_three = nn.utils.spectral_norm(nn.Linear(weight_dim2, 1, bias=False))

        self.kfc_one = nn.utils.spectral_norm(nn.Linear(weight_dim0, 1, bias=False))
        self.kfc_two = nn.utils.spectral_norm(nn.Linear(weight_dim1, 1, bias=False))
        self.kfc_three = nn.utils.spectral_norm(nn.Linear(weight_dim2, 1, bias=False))
    ...
    def forward(self, x, y):
        ...
        if L==3:
            q_weight = list(self.qfc_one.parameters())[0]
            q = x * q_weight.unsqueeze(2).unsqueeze(3)
            k_weight = list(self.kfc_one.parameters())[0]
            k = y * k_weight.unsqueeze(2).unsqueeze(3)
        elif L==128:
            q_weight = list(self.qfc_two.parameters())[0]
            q = x * q_weight.unsqueeze(2).unsqueeze(3)
            k_weight = list(self.kfc_two.parameters())[0]
            k = y * k_weight.unsqueeze(2).unsqueeze(3)
        else:
            q_weight = list(self.qfc_three.parameters())[0]
            q = x * q_weight.unsqueeze(2).unsqueeze(3)
            k_weight = list(self.kfc_three.parameters())[0]
            k = y * k_weight.unsqueeze(2).unsqueeze(3)
