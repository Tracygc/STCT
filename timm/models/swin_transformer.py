...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x_list):
        ...
        # x = torch.flatten(x, 1)
        x_list[0] = self.patch_embed(x_list[0])
        if self.absolute_pos_embed is not None:
            x_list[0] = x_list[0] + self.absolute_pos_embed
        x_list[0] = self.pos_drop(x_list[0])

        x_list = self.layers(x_list)
        del x_list[0]
        list_a = x_list[0]
        list_b = x_list[1]
        list_c = x_list[2]
        list_d = x_list[3]
        list_a = self.norm(list_a.view(1, -1, 1024))
        list_b = self.norm(list_b.view(1, -1, 1024))

        x_list[0] = list_a.view(1, -1, 256)
        x_list[1] = list_b.view(1, -1, 512)
        x_list[2] = self.norm(list_c)
        x_list[3] = self.norm(list_d)
